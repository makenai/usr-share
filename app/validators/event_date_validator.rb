class EventDateValidator < ActiveModel::EachValidator
  ...
  def validate_each(object, attribute, value)
    ...
    same_day_events = object.other.where('member_id = ? AND substr(start_time::varchar,0,11) = ?',
    ...
    same_month_events = object.other.where('member_id = ? AND substr(start_time::varchar,0,8) = ?', 
